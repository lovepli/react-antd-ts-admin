...
import { Tree, Popconfirm, message, Input } from 'antd'
...
class EditableTree extends React.Component<IProps, IState> {
  ...
  // 渲染树
  private renderTreeNodes = (nodes: INode[]) =>
    nodes.map((node) => {
      if (node.isEditable) {
        node.title = (
          <Input
        ...
        node.title = (
          <div className="tree-title">
            ...
            <span className="tree-title__handle">
              <EditOutlined
                style={{ marginLeft: 10 }}
                onClick={this.handleEdit.bind(this, node.id, this.nodes)}
              />
              <PlusCircleOutlined
                style={{ marginLeft: 10 }}
                onClick={this.handleAdd.bind(this, node.id, this.nodes)}
              />
