...
class ColumnBuilder {
  private columnDefines: IColumnProps[] = [];
  ...
  public AddSortNum = (pageNumber: number, pageSize: number, width?: string | number) => {
    const col: IColumnProps = {
      ...
      title: "序号",
      width,
      render: (value: any, record: object, index: number) => (pageNumber - 1) * pageSize + index + 1,
    };
  ...
  public addCodeIdToName = (title: string, filed: string, codeTableName: string, width?: string | number) => {
    const column: IColumnProps = {
      dataIndex: filed,
      title,
      width,
      render: (value: string): React.ReactNode => $codeTable.getNameById(codeTableName, value),
    };
    this.columnDefines.push(column);
  };

  // 将codeTable中的多个id转换为对应的名称
  public addConstantIdsToName = (title: string, filed: string, codeTableName: string, width?: string | number) => {
    const column: IColumnProps = {
      dataIndex: filed,
      title,
      width,
      render: (value: string[]): React.ReactNode =>
        value.map((item) => $codeTable.getNameById(codeTableName, item)).toString(),
    };
    this.columnDefines.push(column);
  };
